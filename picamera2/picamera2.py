...
class CameraManager:
    ...
    def listen(self):
        ...
        while self.running:
            ...
            for key, _ in events:
                callback = key.data
                callback(self.cameras)

    def handle_request(self, cameras):
        """Handle requests
        ...
        for req in self.cms.get_ready_requests():
            if req.status == libcamera.Request.Status.Complete:
                cams.add(req.cookie)
                with cameras[req.cookie]._requestslock:
                    cameras[req.cookie]._requests += [CompletedRequest(req, cameras[req.cookie])]
        for c in cams:
            cameras[c].notifyme_w.write(b"\x00")
            cameras[c].notifyme_w.flush()
...
class Picamera2:
    ...
    def __init__(self, camera_num=0, verbose_console=None, tuning=None):
        ...
        self.notifyme_r, notifymew = os.pipe()
        self.notifyme_rr = os.fdopen(self.notifyme_r, 'rb')
        self.notifyme_w = os.fdopen(notifymew, 'wb')
        self.camera_manager = Picamera2._cm.cms
        self.camera_idx = camera_num
        self._cm.cameras[camera_num] = self
    ...
    def create_preview_configuration(self, main={}, lores=None, raw=None, transform=libcamera.Transform(), colour_space=libcamera.ColorSpace.Sycc(), buffer_count=4, controls={}, display="main", encode="main"):
        ...
        ctrls = set([ctrl.name for ctrl in self.camera.controls.keys()])
        if "NoiseReductionMode" in ctrls and "FrameDurationLimits" in ctrls:
