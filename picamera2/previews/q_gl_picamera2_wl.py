...
def _get_qglpicamera2_wl(qt_module: _QT_BINDING):
    ...
    class QGlPicamera2Wl(QOpenGLWidget):
        ...
        def initializeGL(self):
            # Qt's context is current here, so eglGetCurrentDisplay() is Qt's
            # EGLDisplay (Wayland under Wayland, X11 under X11/XWayland); fall
            # back to the default display if PyOpenGL can't see a current one.
            #
            # PyOpenGL's lazy extension check for eglCreateImageKHR queries
            # `eglGetCurrentDisplay() or eglGetDisplay(EGL_DEFAULT_DISPLAY)` for
            # EGL_EXTENSIONS and raises EGL_NOT_INITIALIZED if that display was
            # never initialised. eglInitialize is idempotent/refcounted, so we
            # initialise it here. Crucially, when no current display is visible
            # this expression resolves to the default display - the very one
            # that fallback uses - so initialising it makes the check pass.
            self.egl_display = eglGetCurrentDisplay() or eglGetDisplay(EGL_DEFAULT_DISPLAY)
            eglInitialize(self.egl_display, EGLint(), EGLint())
            n = GLint()
