...
def _get_qglpicamera2_wl_direct(qt_module: _QT_BINDING):
    ...
    class _GlWindow(QOpenGLWindow):
        ...
        def initializeGL(self):
            # Use Qt's current EGL display, falling back to the default display
            # if PyOpenGL can't see a current one. PyOpenGL's lazy extension
            # check for eglCreateImageKHR queries `eglGetCurrentDisplay() or
            # eglGetDisplay(EGL_DEFAULT_DISPLAY)` and raises EGL_NOT_INITIALIZED
            # if that display was never initialised. eglInitialize is
            # idempotent/refcounted; initialising the resolved display (which is
            # the default display exactly when the fallback would be used) makes
            # the check pass.
